...
from apps.recommendations.models import RecommendationSession, Feedback
...
def generate_plan(request):
    ...
    session = criar_plano_para_usuario(profile)
    if session:
...
def plan_detail(request, session_id):
    ...
    return render(request, 'recommendations/plan_detail.html', {'session': session})

@login_required
def feedback_create(request, session_id, item_id):
    session = get_object_or_404(RecommendationSession, id=session_id, profile__user=request.user)
    item = get_object_or_404(ResourceItem, id=item_id)
